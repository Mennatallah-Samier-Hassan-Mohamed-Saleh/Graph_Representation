  ...
  EdgeList ReadInMTX(std::ifstream &in, bool &needs_weights) {
    ...
    while (std::getline(in, line)) {
      ...
      if (line.empty())
        continue;
      std::istringstream edge_stream(line);
      NodeID_ u; //int in case of bfs run
      //std::cout << "Type of NodeId is: "<<typeid(NodeID_).name() << std::endl;
      edge_stream >> u; //u is storing the first column
      ...
      if (read_weights) {
        ...
        NodeID_ v;
        edge_stream >> v; //vs is storing the second column
