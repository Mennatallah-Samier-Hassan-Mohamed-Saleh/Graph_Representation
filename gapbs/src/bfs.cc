...
{
  // std::cout<<"DOBFS function is called : "<<std::endl;
  if (logging_enabled)
                 ...
                 const pvector<NodeID> &parent)
{
  ...
  std::cout << "Parent size is : " << parent.size() << std::endl;
  ...
  ofstream MyFile("output_gabs");
  // ofstream MyFile("output_gabs_small");
  ...
  ofstream MyFile2("output_distance");
  ...
  MyFile2 << "Printing the Distance Array" << endl;
  for (int i = 0; i < g.num_nodes(); i++)
  {
    cout << distance[i] << " ";
    MyFile2 << distance[i] << " ";
...
int main(int argc, char *argv[])
{
  ...
  if (!cli.ParseArgs())
    return -1;
  /*Create a BuilderBase from the cli*/
  Builder b(cli);
  /*Make the Graph*/
  Graph g = b.MakeGraph();
  std::cout << "Just after initialization" << std::endl;
